...
func TestFloatFields(t *testing.T, hashers ProtoHashers) {
	...
	testCases := []testCase{
		...
		// test fractions to have 32-bit and 64-bit representations that are equal.
		{
			...
			equivalentObject:     map[string][]float64{"values": []float64{0.0078125, 7.888609052210118e-31}},
...

		{
			...
			equivalentObject:     map[string][]float64{"values": []float64{-1.0, 1.5, 1000.000244140625, 1267650600228229401496703205376, 32.0, 13.0009765625}},
